...
const Foyer = () => {
  ...
  const fetchFoyers = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:8222/foyer', getAuthHeader());
      setFoyers(res.data);
    } catch (err) {
  ...
  const addFoyer = async () => {
    try {
      await axios.post('http://localhost:8222/foyer', newFoyer, getAuthHeader());
  ...
  const deleteFoyer = async (id) => {
    try {
      await axios.delete(`http://localhost:8222/foyer/${id}`, getAuthHeader());
      fetchFoyers();
    } catch (err) {
